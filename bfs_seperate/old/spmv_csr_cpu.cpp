...
cmdQueue_cpu =cmdQueue;
    ...
    devresin = clCreateBuffer(context, CL_MEM_READ_WRITE|CL_MEM_ALLOC_HOST_PTR, sizeof(int)*rownum, NULL, &errorCode); CHECKERROR;
    errorCode = clEnqueueWriteBuffer(cmdQueue, devresin, CL_TRUE, 0, sizeof(int)*rownum, res0, 0, NULL, NULL); CHECKERROR;
    devresout = clCreateBuffer(context, CL_MEM_READ_WRITE|CL_MEM_ALLOC_HOST_PTR, sizeof(int)*rownum, NULL, &errorCode); CHECKERROR;
...
//        int threadsinrow=16;
	...
	for (int groupnum = 24; groupnum <= 28; groupnum+= 24)
	{
	    size_t globalsize[] = {4, 1};
