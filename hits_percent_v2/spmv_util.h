...
#define CHECKERROR {if (errorCode != CL_SUCCESS) {fprintf(stderr, "Error at line %d code %d message %s\n", __LINE__, errorCode, print_cl_errstring(errorCode)); exit(1);}}
 ...
 extern void two_vec_compare(float* coovec, float* newvec, int size);
...
//    if (!if_sorted_coo(source)){printf("zf not sorted\n");exit(-1);}//zf added
  ...
  //  int zf=0;
   // int zf2=0;
	...
	while (source->coo_row_id[row] == curRow && row < nnz)
	    row++;              
//        zf2++;if(curRow%100000==0){printf("row=%d zf=%d curRow=%d\n",row, zf, curRow); zf++;}//zf added
 //       if(curRow>(source->matinfo.height)){printf("curRow=%d, row=%d, nnz=%d, height=%d\n",curRow,row,nnz,source->matinfo.height);exit(-1);}//zf added
	curRow++;
