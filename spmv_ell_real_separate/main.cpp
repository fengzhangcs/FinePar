...
double get_time2(cl_event gpuEvt, cl_event cpuEvt, int ret){
  ...
  while(eventStatus != CL_COMPLETE)
  {
    status = clGetEventInfo(
        ...
        &eventStatus,
        NULL);
    if (CL_SUCCESS != status){printf("error!a %d\n", status);exit(-1);}
  }
  ...
  while(eventStatus != CL_COMPLETE)
  {
    status = clGetEventInfo(
        ...
        &eventStatus,
        NULL);
    if (CL_SUCCESS != status){printf("error!b %d\n", status);exit(-1);}
  }
  ...
  status = clGetEventProfilingInfo(cpuEvt,
      ...
      &startTime1,
      0);  
    if (CL_SUCCESS != status){printf("error!c %d\n", status);exit(-1);}
  status = clGetEventProfilingInfo(cpuEvt,
      CL_PROFILING_COMMAND_END,
      sizeof(cl_ulong),
      &endTime1,
      0);  
    if (CL_SUCCESS != status){printf("error!d %d\n", status);exit(-1);}
  ...
  status = clGetEventProfilingInfo(gpuEvt,
      ...
      &startTime0,
      0);
    if (CL_SUCCESS != status){printf("error!e %d\n", status);exit(-1);}
  status = clGetEventProfilingInfo(gpuEvt,
      CL_PROFILING_COMMAND_END,
      sizeof(cl_ulong),
      &endTime0,
      0);
    if (CL_SUCCESS != status){printf("error!f %d\n", status);exit(-1);}
...
void cl_initialization_fusion()
{
	...
	clCommandQue[0] = clCreateCommandQueue(clGPUContext, device_id[0], CL_QUEUE_PROFILING_ENABLE, &errcode);
	if(errcode != CL_SUCCESS) printf("Error in creating command queue\n");
	clCommandQue[1] = clCreateCommandQueue(clGPUContext, device_id[1], CL_QUEUE_PROFILING_ENABLE, &errcode);
	if(errcode != CL_SUCCESS) printf("Error in creating command queue\n");
...
double cputime=0;
    ...
    {
            err = clEnqueueNDRangeKernel(clCommandQue[0], ellpackr, 1, NULL,
                &globalWorkSize, &localWorkSize, 0, NULL, &(e[0]));
      CL_CHECK_ERROR(err);
	    err= clEnqueueNDRangeKernel(clCommandQue[1], csrKernelcpu, work_dimcpu, NULL, &globalsizecpu, &localsizecpu, 0, NULL, &(e[1])); 
      CL_CHECK_ERROR(err);

      err = clFinish(clCommandQue[0]);
      err = clFinish(clCommandQue[1]);
              cputime+= get_time2(e[0],e[1],1);
    }
      CL_CHECK_ERROR(err);
	t_end = rtclock();
        //double cputime=(t_end - t_start)/iters*1000;
...
double gputime=0;
    ...
    for (int j = 0; j < iters; j++)
    {
      ...
      err = clFinish(clCommandQue[0]);
      err = clFinish(clCommandQue[1]);
              gputime+= get_time2(e[0],e[1],0);
    }
      CL_CHECK_ERROR(err);
	t_end = rtclock();
       // double gputime=(t_end - t_start)/iters*1000;

        printf("\n\nCPUTIMEinms: %f GPUTIMEinms: %f cpuoffset: %d\n\n",cputime, gputime, cpuoffset);
