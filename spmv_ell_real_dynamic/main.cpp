...
void *thread1(void *)
{
  ...
  for (i = 0; i < MAX; i++)
  {
    ...
    if(number2>number1+1){
     number1++;
    err= clSetKernelArg(ellpackr, 9, sizeof(int), &number1); 
    CL_CHECK_ERROR(err);
...
//  printf("thread2 : I'm thread 2\n");
 ...
 //   printf("thread2 : number2 = %d\n",number2);
    ...
    if(number2>number1+1){
      number2--;
    err= clSetKernelArg(csrKernelcpu, 11, sizeof(int), &number2); 
    CL_CHECK_ERROR(err);
...
/////////////////////////////////////CPU Side////////////////////////
    ...
    err = clFinish(clCommandQue[0]);
    CL_CHECK_ERROR(err);
    ellpackr = clCreateKernel(clProgram, "spmv_ellpackr_kernel", &err);
    ...
    /////////////////////////////////////////////////////////////////////
    for(int i=0; i<11; i++){
      rowInfo[i]=(double)i/10*cmSize;
    }
    cl_mem devrowInfo;
    devrowInfo = clCreateBuffer(clGPUContext, CL_MEM_READ_ONLY, 11*sizeof(int), NULL, &err);
    CL_CHECK_ERROR(err);//zfadded
    err = clEnqueueWriteBuffer(clCommandQue[0], devrowInfo , true, 0, 11*sizeof(int), rowInfo, 0, NULL, NULL);//zfadded
    CL_CHECK_ERROR(err);
    //	ALLOCATE_GPU_READ(devrowInfo, rowInfo, sizeof(int)*(11));
    err= clSetKernelArg(ellpackr, 8, sizeof(cl_mem), &devrowInfo); 
    CL_CHECK_ERROR(err);
    err= clSetKernelArg(ellpackr, 9, sizeof(int), &number1); 
    CL_CHECK_ERROR(err);
    /////////////////////////////////////////////////////////////////////
...
//        cl_uint work_dimcpu=1;
    ...
    err= clSetKernelArg(csrKernelcpu, 10, sizeof(cl_mem), &devrowInfo); 
    CL_CHECK_ERROR(err);
    err= clSetKernelArg(csrKernelcpu, 11, sizeof(int), &number2); 
    CL_CHECK_ERROR(err);
