...
//#ifdef	PROFILING
		...
		_clMemcpyH2D(d_cost, no_of_nodes*sizeof(int), h_cost);	
                ...
                for(int i=0; i<11; i++){
                  rowInfo[i]=(double)i/10*no_of_nodes;
                }
                cl_mem devrowInfo= _clMalloc(11*sizeof(int), rowInfo);
                _clSetArgs(0, 7, devrowInfo);
                _clSetArgs(0, 8, &number1, sizeof(int));

                _clSetArgs(2, 7, devrowInfo);
...
//#endif
		do{
			...
			_clInvokeKernel_fusion(kernel_id,kernel_id2, no_of_nodes, work_group_size);
...
//		source=0;
                while(h_graph_nodes[source].no_of_edges==0)
                  source++;
                printf("source=%d edgesfromsource=%d\n",source,h_graph_nodes[source].no_of_edges);
