...
void run_bfs_gpu(int no_of_nodes, Node *h_graph_nodes, int edge_list_size, \
	...
	cl_mem d_graph_nodes, d_graph_edges, d_graph_mask, d_updating_graph_mask, \
        ...
        cl_mem d_graph_nodes_cpu ,d_graph_edges_cpu;
	try{
...
//////////////////////////////////////////////////////////////////////////////////////
  ...
  costtime3=timestamp();//costtime
        ...
        unsigned long *bitmap=(unsigned long*)malloc(sizeof(unsigned long)*(((no_of_nodes)>>6) + 1));
	int* rowforcpu = (int*)malloc(sizeof(int)*(no_of_nodes));
        int rowforcpusum=0;
        memset(bitmap,0,(sizeof(unsigned long)*(((no_of_nodes)>>6) + 1)));

        //cpuoffset=2048;
        for(int i=0 ; i<no_of_nodes; i++){
          int numi=h_graph_nodes[i].no_of_edges;//rowptrpad[i+1]-rowptrpad[i];
          if(numi<=cpuoffset){
          //if(numi<=cpuoffset){
          //if(numi>=WARPSIZE){
...
//            printf("%d, ",i);
	...
	cl_mem devbitmap;
//	ALLOCATE_GPU_READ(devbitmap, bitmap, sizeof(unsigned long)*(((no_of_nodes)>>6)+1));
        devbitmap=_clMalloc(sizeof(unsigned long)*(((no_of_nodes)>>6)+1), bitmap);

	cl_mem devrowforcpu;
	//ALLOCATE_GPU_READ_cpu(devrowforcpu, rowforcpu, sizeof(int)*(rowforcpusum));
        devrowforcpu=_clMalloc_cpu(sizeof(int)*(rowforcpusum), rowforcpu);
...
//////////////////////////////////////////////////////////////////////////////////////
		...
		d_graph_nodes_cpu = _clMalloc_cpu(no_of_nodes*sizeof(Node), h_graph_nodes);
		d_graph_edges_cpu = _clMalloc_cpu(edge_list_size*sizeof(int), h_graph_edges);
		...
		_clMemcpyH2D(d_graph_nodes_cpu, no_of_nodes*sizeof(Node), h_graph_nodes);
		_clMemcpyH2D(d_graph_edges_cpu, edge_list_size*sizeof(int), h_graph_edges);	
...
//#endif
		...
		do{
			...
			_clSetArgs(kernel_id, kernel_idx++, devbitmap);//zf
			
			int kernel_id2 = 2;
			kernel_idx = 0;
			_clSetArgs(kernel_id2, kernel_idx++, d_graph_nodes_cpu);
			_clSetArgs(kernel_id2, kernel_idx++, d_graph_edges_cpu);
			_clSetArgs(kernel_id2, kernel_idx++, d_graph_mask);
			_clSetArgs(kernel_id2, kernel_idx++, d_updating_graph_mask);
			_clSetArgs(kernel_id2, kernel_idx++, d_graph_visited);
			_clSetArgs(kernel_id2, kernel_idx++, d_cost);
			_clSetArgs(kernel_id2, kernel_idx++, &no_of_nodes, sizeof(int));

			_clSetArgs(kernel_id2, kernel_idx++, devrowforcpu);//zf
			_clSetArgs(kernel_id2, kernel_idx++, &rowforcpusum, sizeof(int));//zf

			
			//int work_items = no_of_nodes;
			_clInvokeKernel_fusion(kernel_id,kernel_id2, no_of_nodes, work_group_size,2);
		...
		//--statistics
		std::cout<<"kernel time(s):"<<kernel_time/100<<std::endl;		
		std::cout<<"CAUTTION: kernel time(ms): "<<kernel_time*10<<std::endl;		// /100*1000=*10
...
//#endif
		...
		//kernel_timer.start();
			...
			kernel_time += (_clInvokeKernel_fusion(kernel_id,kernel_id2, no_of_nodes, work_group_size,0));
			//_clInvokeKernel(kernel_id, no_of_nodes, work_group_size);
		//	kernel_timer.stop();
		//kernel_time += kernel_timer.getTimeInSeconds();
		//	_clInvokeKernel_fusion(kernel_id,kernel_id2, no_of_nodes, work_group_size,1);
		...
		_clMemcpyD2H(d_cost,no_of_nodes*sizeof(int), h_cost);
		//--statistics
          double onegputime = kernel_time/100;
...
//#endif
		...
		//kernel_timer.start();		
			//int work_items = no_of_nodes;
			kernel_time += _clInvokeKernel_fusion(kernel_id,kernel_id2, no_of_nodes, work_group_size,1);
			//_clInvokeKernel(kernel_id, no_of_nodes, work_group_size);
		//	kernel_timer.stop();
		//kernel_time += kernel_timer.getTimeInSeconds();
		//	_clInvokeKernel_fusion(kernel_id,kernel_id2, no_of_nodes, work_group_size,0);
		...
		_clMemcpyD2H(d_cost,no_of_nodes*sizeof(int), h_cost);
		//--statistics
          double onecputime = kernel_time/100;



        printf("\n\nCPUTIMEinms: %f GPUTIMEinms: %f cpuoffset: %d\n\n",onecputime, onegputime, cpuoffset);
...
fprintf(stderr,"Usage: %s <input_file> percentage\n", argv[0]);
    ...
    cpuoffset = atoi(argv[2]);
		...
		for(int i = 0; i < no_of_nodes; i++){
			...
			h_graph_visited[i]=false;
                ...
                printf("maxedgeno is %d\n",maxedgeno);
		//read the source node from the file
		fscanf(fp,"%d",&source);
//		source=0;
                while(h_graph_nodes[source].no_of_edges==0)
                  source++;
                printf("source=%d\n",source);
