...
double _clInvokeKernel_fusion(int kernel_id, int kernel_id2, int work_items, int work_group_size, int choose) throw(string){
	cl_uint work_dim = WORK_DIM;
        ...
        if(choose==2){
          ...
          oclHandles.cl_status = clEnqueueNDRangeKernel(oclHandles.queue[1], \
              oclHandles.kernel[kernel_id2], work_dim, NULL, \
              globalcpu, globallocal, 0 , 0, &(e[1]) );	
