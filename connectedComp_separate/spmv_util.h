...
extern void spmv_csr_vector_ocl(csr_matrix<int, float>* mat, int* parents, int* shadow, char* mask, int* edge_src, int padNum, double& opttime, int& optmethod, char* oclfilename, int ntimes);
...
#define ALLOCATE_GPU_READ(deviceBuf, HostBuf, mem_size) \
    ...
    if( errorCode != CL_SUCCESS ) { printf("Error: clCreateBuffer returned %d line %d\n", errorCode,__LINE__);exit(-1); } \
    }
