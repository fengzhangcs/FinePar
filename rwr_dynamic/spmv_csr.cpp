  ...
  for (i = 0; i < MAX; i++)
  {
    ...
    if(number2>number1+1){
     number1++;
              ...
              errorCode = clSetKernelArg(csrKernel, 7, sizeof(cl_mem), &devrold); CHECKERROR;//added
              errorCode = clSetKernelArg(csrKernel, 8, sizeof(cl_mem), &devrnew); CHECKERROR;//added
    ...
    if(number2>number1+1){
      number2--;
        ...
        errorCode = clSetKernelArg(csrKernelcpu, 12, sizeof(int), &number2); CHECKERROR;
              ...
              errorCode = clSetKernelArg(csrKernelcpu, 8, sizeof(cl_mem), &devrold); CHECKERROR;//added
              errorCode = clSetKernelArg(csrKernelcpu, 9, sizeof(cl_mem), &devrnew); CHECKERROR;//added
...
//    cl_mem devrnew;
 //   cl_mem devrold;
    ...
    float *tmptrori= (float*)malloc(sizeof(float)*(mat->matinfo.width));
    ...
    devrold= clCreateBuffer(context, CL_MEM_READ_WRITE|CL_MEM_ALLOC_HOST_PTR, sizeof(float)*vecsize, NULL, &errorCode); CHECKERROR;
    errorCode = clEnqueueWriteBuffer(cmdQueue, devrold, CL_TRUE, 0, sizeof(float)*rownum, tmptrori, 0, NULL, NULL); CHECKERROR;
    devrnew= clCreateBuffer(context, CL_MEM_READ_WRITE|CL_MEM_ALLOC_HOST_PTR, sizeof(float)*vecsize, NULL, &errorCode); CHECKERROR;
    errorCode = clEnqueueWriteBuffer(cmdQueue, devrnew, CL_TRUE, 0, sizeof(float)*rownum, tmptrori, 0, NULL, NULL); CHECKERROR;
    free(tmptrori);
...
//	    size_t globalsize[] = {groupnum*CSR_VEC_GROUP_SIZE, dim2};
	    ...
	    errorCode = clEnqueueWriteBuffer(cmdQueue, devRes, CL_TRUE, 0, sizeof(float)*rownum, result, 0, NULL, NULL); CHECKERROR;
            errorCode = clEnqueueWriteBuffer(cmdQueue, devrold, CL_TRUE, 0, sizeof(float)*rownum, vec, 0, NULL, NULL); CHECKERROR;
            errorCode = clEnqueueWriteBuffer(cmdQueue, devrnew, CL_TRUE, 0, sizeof(float)*rownum, vec, 0, NULL, NULL); CHECKERROR;
            clFinish(cmdQueue);
...
/*
	    ...
	    errorCode = clEnqueueReadBuffer(cmdQueue, devrold, CL_TRUE, 0, sizeof(float)*rownum, tmpresult, 0, NULL, NULL); CHECKERROR;
	    clFinish(cmdQueue);
...
//    printf("cpuoffset=%d\n",cpuoffset);
    ...
    memset(tmp,0,sizeof(float)*mat.matinfo.width);
    for(int i=0; i<mat.matinfo.nnz; i++){
      tmp[mat.coo_col_id[i]]++;
    }
    for(int i=0; i<mat.matinfo.nnz; i++){
      mat.coo_data[i]=1.0/tmp[mat.coo_col_id[i]];
    }
   ...
   {
	sprintf(clfilename, "%s%s", "/home/pacman/zf/wubo/apu_corun/rwr_dynamic", "/spmv_csr_vector.cl");
    printMatInfo(&mat);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
    int sourceid=0;
    float* rnew = (float*)malloc(sizeof(float)*mat.matinfo.width);
    float* rold = (float*)malloc(sizeof(float)*mat.matinfo.width);
    float* ei = (float*)malloc(sizeof(float)*mat.matinfo.width);
    float tmpt=1.0;
...
//        printf("row=%d start\n",row);
        for(int j=start; j<end; j++){
          ...
          accumulant += data*rold[col];
          //accumulant += data*prold[j];
        }
        accumulant = accumulant*d + (1-d)*ei[row];
        rnew[row]=accumulant;
...
*/
    ...
    spmv_csr_vector_ocl(&csrmat, ei, res, 0,  opttime1, optmethod1, clfilename, coores, ntimes);//zf method1
	...
	spmv_csr_vector_ocl(&padcsr, ei, res, 16, opttime2, optmethod2, clfilename, coores, ntimes);
