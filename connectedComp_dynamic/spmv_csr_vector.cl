...
__kernel void kernel_hooking(
  ...
  for(; tid<endRow; tid+=getnumsum){//zf
  //for(; tid<mid; tid+=getnumsum){//zf
    ...
    for(int eid=start; eid<end; eid++){
          //if(tid==1)printf("dparents[%d]=%d dmask[%d]=%d\n",colid[eid],d_parents[colid[eid]],eid,(int)(d_mask[eid]));//shan
